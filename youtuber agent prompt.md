...
here’s some of the stuff i want you to do, create the responsibilities, approach, constraints, output format from these guidelines

let’s work together on this, so please give me some back and forth if you need it

you will help me post and publish videos in YouTube studio under my brand account [brand], under my google account [google account]
...
7. this smaller compressed file goes to my hard drive
8. then i make a copy of the compressed file with no audio and append the label “ytready” to it — this goes to youtube studio
9. at this point, we have a bunch of videos that are being uploaded that have “ytready” suffixed (no audio), and the compressed videos (with audio) that have been saved on the hard drive
10. at this point, i need you to ask me if there was teaching footage here because it needs audio (or maybe you can ask earlier if you think it makes more computational sense), and i need to let you know which days and time they are so you can make another copy of those compressed videos with audio and rename them to YYYY-MM-DD-teaching-class-fc.mov
11. we then take those teaching videos and upload to youtube studio and also save a copy in the hard drive in a different location
12. we wipe the slate clean of any files that have been finished uploading or finished saving on the hard drive, and we’re ready for the next batch.
...
- human intervention is still needed for delineating which gym it is, and certain videos are only of teaching, not just completely rolling
	- airdropping and organizing files into the correct folders, still a pain
- `--convert-only` and `--upload-only` were added to `process.sh` so we can deal with error handling when yt determines our `request.token` is dead
- the most annoying part is still the most difficult to automate due to api rate limits and request.tokens needing refreshing
	- maybe we can refresh the tokens automatically somehow?
